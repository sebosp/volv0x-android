...
fun FindBLEDevicesSample() {
    BluetoothSampleBox {
        FindDevicesScreen {
            Log.d("onConnect()", "Name: ${it.name} Address: ${it.address} Type: ${it.type}")
        }
    }
}
...
internal fun FindDevicesScreen(onConnect: (BluetoothDevice) -> Unit) {
    ...
    val distanceSensorDevices = remember {
        mutableStateListOf<BluetoothDevice>()
    }
    ...
    if (scanning) {
        BluetoothScanEffect(
            ...
            onDeviceFound = { scanResult ->
                if (!devices.contains(scanResult.device) && scanResult.device.name != null && scanResult.device.name.startsWith("SEB-PATCH")) {
                ...
                if (serviceUuids.contains(ParcelUuid(SERVICE_UUID))) {
                    if (!distanceSensorDevices.contains(scanResult.device)) {
                        Log.i("FindBLEDevicesSample", "Adding device to distanceSensorDevices")
                        distanceSensorDevices.add(scanResult.device)
                    }
                }
            },
        )
        ...
        LazyColumn(
            ...
            items(devices) { item ->
                BluetoothDeviceItem(
                    ...
                    isDistanceSensor = distanceSensorDevices.contains(item),
                    onConnect = onConnect,
                )
            }
...
internal fun BluetoothDeviceItem(
    ...
    isDistanceSensor: Boolean = false,
...
//        horizontalArrangement = Arrangement.SpaceBetween,
//    ) {
//        ConnectDeviceScreen(device = bluetoothDevice) {}
//    }
}
